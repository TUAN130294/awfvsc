...
**Đọc `personality` từ ~/.claude/CLAUDE.md (user preferences) để điều chỉnh cách code:**
...
```json
// CLAUDE.md + .claude/rules/session.json
{
  "working_on": {
...
4. Folder structure:
   ...
   - Tạo CLAUDE.md + .claude/rules/ folder
...
2️⃣ Nghỉ ngơi? `/awf:recap` để lưu progress
...
Sau mỗi task, APPEND 1 dòng vào `CLAUDE.md + .claude/rules/session_log.txt`:
...
### Sau thay đổi lớn:
*   "Đây là thay đổi quan trọng. Nhớ `/awf:recap` cuối buổi!"

### Sau thay đổi security-sensitive:
*   "Em đã thêm security measures. Anh có thể `/awf:audit` để kiểm tra thêm."

### Sau hoàn thành phase:
*   "Phase xong rồi! `/awf:recap` để lưu trước khi nghỉ nhé."
...
3️⃣ Xem progress? `/awf:next`
4️⃣ Lưu context? `/awf:recap`
```
